...
- **Safe type-cast joins**: `Join` accepts optional `localKeyType` (e.g. `'uuid'`, `'integer'`). When set, `SelectStatement` renders the local key as `NULLIF("table"."col", '')::type` — converting empty strings to `NULL` before casting to prevent PostgreSQL errors like `invalid input syntax for type uuid: ""`. Without `localKeyType`, the standard `"table"."col"` form is used.
