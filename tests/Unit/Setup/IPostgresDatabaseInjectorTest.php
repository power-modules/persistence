...
namespace Modular\Persistence\Tests\Unit\Setup;
...
final class IPostgresDatabaseInjectorTest extends TestCase
{
    public function testSetupDoesNothingWhenPhaseIsNotPost(): void
    {
        ...
        $rootContainer->expects(self::never())->method('get');
        $moduleContainer->expects(self::never())->method('set');
    ...
    public function testSetupInjectsDatabaseWhenPhaseIsPost(): void
    {
        ...
        $rootContainer->expects(self::once())
            ->method('get')
            ->with(IPostgresDatabase::class)
            ->willReturn($database);

        $moduleContainer->expects(self::once())
            ->method('set')
