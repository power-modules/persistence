...
final class IndexTest extends TestCase
{
    ...
    {
        $index = new Index(['id', 'name'], null, false);
        self::assertSame('idx_296856443', $index->makeName(''));
    }
    ...
    {
        $columns = ['col_a', 'col_b'];
        $btree = new Index($columns, null, false, IndexType::Btree);
        $gin = new Index($columns, null, false, IndexType::Gin);

        self::assertNotSame($btree->makeName('table'), $gin->makeName('table'));
    }

    public function testMakeNameShouldBeConsistentForBtreeDefault(): void
    {
        $columns = ['id', 'name'];
        $explicit = new Index($columns, null, false, IndexType::Btree);
        $default = new Index($columns, null, false);

        self::assertSame($explicit->makeName(''), $default->makeName(''));
    }

    public function testMakeNameShouldIncludeTableName(): void
    {
        $index = new Index(['col_a'], null, false, IndexType::Hash);
        self::assertStringStartsWith('idx_users_', $index->makeName('users'));
    }
